...
use hall::message::{GameEndTurnRequest, GameEndTurnResponse, GameTickMessage};
use shared_net::types::NodeType;
use shared_net::{op, RoutedMessage, VSizedBuffer};
use tokio::sync::mpsc::UnboundedSender;

pub(crate) fn recv_game_end_turn(context: HallContext, tx: UnboundedSender<RoutedMessage>, mut buf: VSizedBuffer) {
    ...
    let success = update_user(&mut context.games, request.game_id, header.user, header.auth, op::Command::GameEndTurn, |_user| {
        /*TODO:*/
        true
    });
