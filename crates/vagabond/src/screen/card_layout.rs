...
impl From<Option<VagabondCard>> for CardPopulateEvent {
    fn from(card: Option<VagabondCard>) -> Self {
        Self {
            card,
        }
    }
}
impl From<&VagabondCard> for CardPopulateEvent {
    fn from(card: &VagabondCard) -> Self {
        Self {
    ...
    fn on_populate(
        ...
        match (&event.card, layout_q.get(target)) {
            (Some(card), Ok(layout)) => {
                ...
                layout.launch.map(|launch| text_q.get_mut(launch).map(|mut launch_text| *launch_text = card.launch_rules.clone().into()));
